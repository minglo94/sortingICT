...
const App = () => {
  ...
  // Compute expected pass-by-pass results for a given array and algorithm
  const computeSortPasses = (inputArr, algo) => {
    const arr = [...inputArr];
    const n = arr.length;
    const passes = {};
    ...
    if (algo === 'insertion') {
      ...
      for (let pass = 1; pass < n; pass++) {
        const element = a[pass];
        let pos = pass;
        while (pos > 0 && element < a[pos - 1]) {
          a[pos] = a[pos - 1];
          pos--;
        }
        a[pos] = element;
  ...
  useEffect(() => {
    resetSort();
    const defaultOriginal = algo === 'merge' ? [8, 18, 12, 7, 15, 1, 3] : [8, 18, 12, 15, 1, 3];
    const computed = computeSortPasses(defaultOriginal, algo);
    setChallengeOriginal(defaultOriginal);
    setExpectedAnswers(computed);
    setChallengeInputs({
      pass1: ["", "", "", "", "", ""], pass2: ["", "", "", "", "", ""],
      pass3: ["", "", "", "", "", ""], pass4: ["", "", "", "", "", ""],
      pass5: ["", "", "", "", "", ""]
  ...
  const generateNewChallenge = async () => {
    setIsAiLoading(true);
    const prompt = `請生成 6 個介於 1 到 50 之間的隨機不重複整數作為「${algo}排序」挑戰題的原始數列。只返回 JSON 格式，包含 'original' 欄位是包含 6 個整數的陣列。不需要其他欄位。`;
    ...
    try {
      ...
      const newOriginal = res.original;
      const computed = computeSortPasses(newOriginal, algo);
      setChallengeOriginal(newOriginal);
      setExpectedAnswers(computed);
      setChallengeInputs({
        pass1: ["", "", "", "", "", ""], pass2: ["", "", "", "", "", ""],
        pass3: ["", "", "", "", "", ""], pass4: ["", "", "", "", "", ""],
